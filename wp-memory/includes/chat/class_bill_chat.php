...
class ChatPlugin
{
    ...
    public function enqueue_chat_scripts()
    {
        ...
        wp_localize_script('chat-script', 'bill_data', array(
            ...
            'invalid_response_format'  => esc_attr__('Invalid response format:', 'wp-memory'),
            'response_processing_error' => esc_attr__('Error processing server response:', 'wp-memory'),
            'not_json'                 => esc_attr__('Response is not valid JSON.', 'wp-memory'),
            'ajax_error'               => esc_attr__('AJAX request failed:', 'wp-memory'),
            'send_error'               => esc_attr__('Error sending the message. Please try again later.', 'wp-memory'),
            'empty_message_error'      => esc_attr__('Please enter a message!', 'wp-memory'),
        ));
    }
    /**
    ...
    public function bill_chat_call_chatgpt_api($data, $chatType, $chatVersion)
    {
        ...
        if (isset($data['success']) && $data['success'] === true) {
            ...
            $message = esc_attr__("Error contacting the Artificial Intelligence (API). Please try again later.", 'wp-memory');
        }
        // debug2($message);
        return $message;
    }
    ...
    public function bill_chat_send_message()
    {
        ...
        if (empty($message)) {
            if ($chatType == 'auto-checkup') {
                $message = esc_attr("Auto Checkup for Erros button clicked...", 'wp-memory');
            } elseif ($chatType == 'auto-checkup2') {
                $message = esc_attr("Auto Checkup Server button clicked...", 'wp-memory');
            }
        }

        //  if (empty($message)) {
        //    $message = esc_attr("Auto Checkup button clicked...", 'wp-memory');
        // }
        ...
        if (!empty($response_data)) {
            ...
            $output = esc_attr__("Error to get response from AI source!", 'wp-memory');
        }
